				...
				<div class="col-md-8">
					...
					<input type="hidden" name="g_no" value="${gBoardVo.g_no}"/>
						<div class="form-group">
							...
							<input type="text" class="form-control" id="g_writer" name="g_writer" value="${gBoardVo.g_writer}" readonly="readonly"/>
							<label for="gBoard_location">여행지역</label><br>
							<select id="g_location" name="g_location">
								<option value="location" >지역선택</option>
								<option value="유럽" <c:if test="${gBoardVo.g_location == '유럽' }">selected="selected"</c:if>>유럽</option>
								<option value="중국/대만" <c:if test="${gBoardVo.g_location == '중국/대만' }">selected="selected"</c:if>>중국/대만</option>
								<option value="동남아/홍콩" <c:if test="${gBoardVo.g_location == '동남아/홍콩' }">selected="selected"</c:if>>동남아/홍콩</option>
								<option value="일본" <c:if test="${gBoardVo.g_location == '일본' }">selected="selected"</c:if>>일본</option>
								<option value="미국/캐나다/특수" <c:if test="${gBoardVo.g_location == '미국/캐나다/특수' }">selected="selected"</c:if>>미국/캐나다/특수</option>
								<option value="제주/내륙" <c:if test="${gBoardVo.g_location == '제주/내륙' }">selected="selected"</c:if>>제주/내륙</option>
							</select>
						</div>
						<div class="form-group">
							 
							<label for="gBoard_title">제목</label>
							<input type="text" class="form-control" id="g_title" name="g_title" value="${gBoardVo.g_title }"/>
						</div>
						<div class="form-group g_content">
							 
							<label for="g_content">내용</label><br>
							<textarea rows="20" id="g_content" name="g_content" style="resize:none;width:100%">${gBoardVo.g_content }</textarea>
						</div>
						<div class="form-group">
